...
export default async (req, res) => {
    ...
    const params = [usertag, hashedPassword, encryptedEmail, fullnamem, encryptedMobile, gender, birthdate];
    const result = await UseDB(sql, params);
    const uploadPfp = req.file;
    if (uploadPfp) {
        const pfpSql = 'INSERT INTO profpics (userid, filename) VALUES (?, ?)';
        const pfpParams = [result.insertId, uploadPfp.filename];
        await UseDB(pfpSql, pfpParams);
    }
